    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-indigo-100 to-blue-200 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-12 px-4">
      <div className="w-full max-w-2xl bg-white/70 dark:bg-gray-900/80 backdrop-blur-md shadow-xl rounded-3xl p-8 sm:p-10 border border-gray-300 dark:border-gray-700 transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {fields.map(({ label, name, type = "text", optional }) => (
            <div key={name} className="relative">
              <input
                ...
                placeholder={label}
              />
              <label className="absolute left-4 top-2 text-gray-600 dark:text-gray-300 text-xs font-medium transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-500 peer-focus:top-2 peer-focus:text-xs peer-focus:text-indigo-600 dark:peer-focus:text-indigo-300">
                {label}
              </label>
            </div>
          ))}
